...
local fonts = {
	Cozette = "https://github.com/slavfox/Cozette/releases/download/v.1.19.1/CozetteVector.otf",
	PlexMono = "https://github.com/IBM/plex/raw/master/IBM-Plex-Mono/fonts/complete/otf/IBMPlexMono-Regular.otf",
	Quicksand = "https://github.com/Rain-Design/Fonts/raw/main/Quicksand-Regular.otf",
	ProggyClean = "https://github.com/bluescan/proggyfonts/raw/master/ProggyVector/ProggyVector Regular.ttf",
	ApathyMono = "https://github.com/technorav3nn/fonts/raw/main/apathy-monospace.otf",
}
...
for font, url in pairs(fonts) do
	if isfile("Fonts" .. font .. ".otf") then
		continue
	end

	local fontRequest = request({
		Url = url,
		Method = "GET",
	})

	writefile("Fonts/" .. font .. ".otf", fontRequest.Body)
end

local function getFont(font)
	assert(fonts[font], "There's no font called: " .. font, 0)

	local fontFace = "Fonts/" .. font .. "Face" .. ".json"
	local fontPath = "Fonts/" .. font .. ".otf"

	if not isfile(fontFace) then
		writefile(
			fontFace,
			httpService:JSONEncode({
				name = font,
				faces = { { name = "Regular", weight = 300, style = "normal", assetId = getcustomasset(fontPath) } },
			})
		)
	end

	return getcustomasset(fontFace)
end
...
local Library = {
	...
	Font = Font.fromEnum(Enum.Font.Code),--[[ Font.new(getFont("ApathyMono")),]] --[[ Enum.Font.Code ]]
	-- sono monospace id: 12187362578
